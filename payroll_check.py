...
def run_checks(salary_df, bank_df, ctc_df, prev_ctc_df, bank_master_df) -> CheckResults:
    ...
    c_r_esi = find_col(salary_df.columns, "esi") or find_col(salary_df.columns, "e s i")
    ...
    for emp_id, cur_fixed, prev_fixed in zip(all_emp_ids, cur_fixed_map, prev_fixed_map):
        if pd.isna(cur_fixed) or pd.isna(prev_fixed):
            continue  # missing from one side handled separately / new joiner, not a "change"
    ...
    has_exception = result[diff_cols].abs().gt(DIFF_TOLERANCE).any(axis=1)
